...
const Camera = (mode = 'Perspective', fov = 50, zoom = 1, near = 0.01, far = 2000, focus = 10, aspect = 1, filmGauge = 35, filmOffset = 0) => (store, transform) => {
    if (!store.hasBind('camera.mode')) {
        store.addBind('camera.mode', mode, Components_1.selectClamper(['Perspective', 'Orthographic']))
    ...
    store.set('camera', {
        ...
        updateProjectionMatrix(view) {
            if (mode !== 'Perspective')
                return;
            let top = self.near * Math.tan(Math.PI / 180 * 0.5 * self.fov) / zoom, height = 2 * top, width = self.aspect * height, left = -0.5 * width;
            if (view !== undefined) {
            ...
            if (skew !== 0)
                left += self.near * skew / this.filmWidth();
