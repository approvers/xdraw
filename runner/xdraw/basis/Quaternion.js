...
class Quaternion {
    ...
    static fromEuler(euler, update = false) {
        if (!(euler instanceof Euler_1.default)) {
    ...
    static fromAxisAngle(axis, angle) {
        const halfAngle = angle / 2, s = Math.sin(halfAngle), newQ = new Quaternion();
        newQ._x = axis.x * s;
        newQ._y = axis.y * s;
        newQ._z = axis.z * s;
    ...
    static fromRotationMatrix(m) {
        const te = m.elements, m11 = te[0], m12 = te[4], m13 = te[8], m21 = te[1], m22 = te[5], m23 = te[9], m31 = te[2], m32 = te[6], m33 = te[10], trace = m11 + m22 + m33, newQ = new Quaternion();
        let s;
        if (trace > 0) {
    ...
    static fromUnitVectors(from, to) {
        let v = new Vector3_1.default(), r = from.dot(to) + 1;
        if (r < 0.000001) {
            r = 0;
            if (Math.abs(from.x) > Math.abs(from.z)) {
                v = new Vector3_1.default(-from.y, from.x, 0);
            }
            else {
                v = new Vector3_1.default(0, -from.z, from.y);
            }
        }
        else {
            v = from.cross(to);
    ...
    rotateTowards(q, step) {
        const oneAngle = step / this.angleTo(q), works = [];
