...
export class Sprite {
  ...
  raycast(raycaster: Raycaster, intersects: RaycastIntersection[]) {
    function transformVertex(
      ...
      const alignedPosition = vertexPosition
        .sub(center)
        .addScalar(0.5)
        .multiply(scale);
    ...
    const viewWorldMatrix = this.transform.matrixWorld.multiply(
      this.transform.modelViewMatrix.inverse(Matrix4.identity()),
    );
    const mvPosition = Vector3.fromMatrixPosition(
      this.transform.modelViewMatrix,
    );

    const { rotation } = this.material.props;
    let sin = 0,
      cos = 0;
    if (rotation !== 0) {
    ...
    let intersectPoint = raycaster.ray.intersectedPointWithTriangle(
    ...
    if (intersectPoint === null) {
      ...
      intersectPoint = raycaster.ray.intersectedPointWithTriangle(
        new Triangle(vA, vC, vB),
        false,
      );
      if (intersectPoint === null) {
        return;
