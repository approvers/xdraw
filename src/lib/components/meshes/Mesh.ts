...
export default class Mesh extends Component {
  packMesh(nums: {
    ...
    this.store.addState<{ [key: string]: BufferAttribute }>({
      index:
        nums.indices && nums.indices.some((v) => v > 65535)
  ...
  apply(
    ...
    const [attributes] = this.store.addState<{
    ...
    Object.keys(attributes).forEach((key) => {
      ...
      gl.vertexAttribPointer(
        attributeLocation,
        attributes()[key].itemSize,
        attributes()[key].isFloat ? gl.FLOAT : gl.INT,
        false,
        0,
        0,
      );
    ...
    // Add IBO
    if (attributes().index && attributes().index.count > 0) {
    ...
    const count = attributes().position ? attributes().position.length : 0;
